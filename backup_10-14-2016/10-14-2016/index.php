...
$app->get('/locationID[/{location_id}]', function ($request, $response, $args) { 
...
// User Edit
$app->post('/user/edit[/{user_id}]', function ($request, $response, $args) {   
	 $parsedBody = $request->getParsedBody(); // print_r($parsedBody); 
	 $Auth = $request->getHeaderLine('Auth-Token');  
...
// get event by id
$app->get('/event[/{ev_id}]', function($request, $response, $args){ 
	$ev_id = $args['ev_id'];
	$Auth = $request->getHeaderLine('Auth-Token');  
	if($ev_id)
	{ 
...
// Get Event Checkin
$app->post('/checkinmember[/{ev_id}]', function ($request, $response, $args) { 
...
// Edit Event
$app->post('/editevent[/{ev_id}]', function ($request, $response, $args) { 
	 $parsedBody = $request->getParsedBody();   // print_r($parsedBody);   
	 $Auth = $request->getHeaderLine('Auth-Token');  
	 $evID = $args['ev_id'];
	 if($Auth)
...
// Edit Location
$app->post('/editlocation[/{loc_id}]', function ($request, $response, $args) { 
	 $parsedBody = $request->getParsedBody();   // print_r($parsedBody);   
	 $Auth = $request->getHeaderLine('Auth-Token');  
	 $LocID = $args['loc_id'];
	 if($Auth)
...
// get Picket by id
$app->get('/getpicket[/{pk_id}]', function($request, $response, $args){ 
	 $pk_id = $args['pk_id'];  
	$Auth = $request->getHeaderLine('Auth-Token');  
	if($pk_id)
	{ 
...
// Edit Picket
$app->post('/editpicket[/{pk_id}]', function ($request, $response, $args) {  
	 $parsedBody = $request->getParsedBody();  //  print_r($parsedBody);  
	 $Auth = $request->getHeaderLine('Auth-Token'); 
	  $pk_id = $args['pk_id']; 
	 if($Auth)
...
// get Picket picketsignups - 5-31-2016
...
// get Picket completedslots - 5-31-2016
$app->post('/completedslots', function($request, $response, $args){  
	$Auth = $request->getHeaderLine('Auth-Token');   
	$parsedBody = $request->getParsedBody();    //print_r($parsedBody);  
...
// Picket Attendence picketattendancesummary - 5-31-2016
...
$app->get('/member[/{member_id}]', function ($request, $response, $args) {  
	// $parsedBody = $request->getParsedBody();  //  print_r($parsedBody);  
	...
	if($Auth) //Auth-Token
	{
		...
		if($check)
		{
			...
			if($udata[0]->role != 3)
			{
				...
				$mem = "select * from members where Emp_No = '".$Member_ID."' AND Company = '".$cdata[0]->Company_Name."'";
				...
				$cdata = $comdata->fetchAll(PDO::FETCH_OBJ);
					...
					if(empty($mdata)){
					   if($First_Name != '' && $Last_Name != '' && $Member_ID != ''){
						...
						/*if($Company == 'CON EDISON') // 8 Digits
						{
						   $memID = "CE".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'CE';
						}
						elseif($Company == 'ENTERGY'){
						   $memID = "IP".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'IP';
						}
						elseif($Company == 'LEWIS TREE'){
						   $memID = "LT".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'LT';
						}
						elseif($Company == 'NAES'){
						   $memID = "NA".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'NA';
						}
						elseif($Company == 'NRG'){
						   $memID = "NRG".str_pad($Member_ID, 7, '0', STR_PAD_LEFT);
						   $prefix = 'NRG';
						}
						elseif($Company == 'NYPA'){
						   $memID = "NYPA".str_pad($Member_ID, 6, '0', STR_PAD_LEFT);
						   $prefix = 'NYPA';
						}
						elseif($Company == 'TRANS CANADA'){
						   $memID = "TC".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'TC';
						}
						elseif($Company == 'UNITED WATER'){
						   $memID = "UW".str_pad($Member_ID, 8, '0', STR_PAD_LEFT);
						   $prefix = 'UW';
						}
						elseif($Company == 'USPG'){
						   $memID = "USPG".str_pad($Member_ID, 6, '0', STR_PAD_LEFT);
						   $prefix = 'USPG';
						}
						elseif($Company == 'UWUA'){
						   $memID = "UWUA".str_pad($Member_ID, 6, '0', STR_PAD_LEFT);
						   $prefix = 'UWUA';
						}
						elseif($Company == 'WRS'){
						   $memID = "WRS".str_pad($Member_ID, 7, '0', STR_PAD_LEFT);
						   $prefix = 'WRS';
						}
						else
						{
...
// Add Member Code
$app->post('/addmember', function ($request, $response, $args) { 
...
// Edit Member Pending...
$app->post('/	[/{member_id}]', function ($request, $response, $args) { 
...
$app->post('/call_in', function ($request, $response, $args) { 
	$parsedBody = $request->getParsedBody();
	$Auth = $request->getHeaderLine('Auth-Token');
	$check = checkAuthKey($Auth);
	$db = getDB();
...
$app->post('/add_call_in', function ($request, $response, $args) { 
	$parsedBody = $request->getParsedBody();
	$Auth = $request->getHeaderLine('Auth-Token');
	$check = checkAuthKey($Auth);
	 $db = getDB();
...
$app->post('/call_in_staff', function ($request, $response, $args) {
...
$app->get('/GetCompanyInfo', function ($request, $response, $args) {  
	    $Auth = $request->getHeaderLine('Auth-Token');
		$db = getDB();   
		if($Auth)
		{ 
