	...
	if($Auth != '') //Auth-Token
	{
		...
		if($check)
		{
			...
			if($udata[0]->role != 3)
			{
				...
				$First_Name = $parsedBody['First_Name'];
				$Last_Name = $parsedBody['Last_Name'];
				$Company = strtoupper($parsedBody['ID_Prefix']);
			...
			if(!$onem)
			{
				...
				if(!empty($cdata)){  
				  if($flag == 0){
					...
					$sql = "insert into member_data(Member_ID,Emp_No,Password,First_Name,Last_Name,Email,Phone,Street_Address,Apt_Suite_Room,City,State,Zip_Code,Company_Prefix) values('$memID','$ID','$Password','$First_Name','$Last_Name','$Email','$Phone','$Street_Address','$Apt_Suite_Room','$City','$State','$Zip','$Company')";    
					$stmt = $db->query($sql); 
					$lastinsert = $db->lastInsertId();    
